...
export default function EditTestimonialForm({
  ...
  testimonial: { id: string; Rating: number; Message: string };
}) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        setState({ success: false, errors: result.errors, message: result.error });
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-[30px] md:p-[40px] rounded-[30px] shadow-[0px_10px_30px_rgba(0,0,0,0.03)] border border-pale-blue">
      {state.success && (
        <div className="mb-6 p-4 bg-[#e8f5e9] text-[#009515] rounded-xl flex items-center gap-2">
          <Icon name="check-circled" width={20} height={20} className="text-[#009515]" />
      ...
      <div className="mb-6">
        <label htmlFor="rating" className="block text-dark-purple font-bold mb-2">
          Star Rating (1-5)
        </label>
        ...
        {state.errors?.rating && (
          <p className="text-[#ff0000] text-sm mt-2 ml-4">{state.errors.rating[0]}</p>
        )}
      </div>

      <div className="mb-[30px]">
        <label htmlFor="message" className="block text-dark-purple font-bold mb-2">
          Testimonial Message
        </label>
        <div className="relative">
          <textarea
        ...
        {state.errors?.message && (
          <p className="text-[#ff0000] text-sm mt-2 ml-4">{state.errors.message[0]}</p>
        )}
      </div>
      ...
      >
        {pending ? "Updating..." : "Update Testimonial"}
        {!pending && (
          <Icon
            name="arrow-right"
            width={20}
