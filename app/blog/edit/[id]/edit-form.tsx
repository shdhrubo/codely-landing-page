...
import { updateBlogAction, FormState } from "@/app/actions/blog/actions";
import Icon from "@/app/shared/icon/IconComponent";

const initialState: FormState = {};

export default function EditForm({ blog }: { blog: { id: string, Name: string, Description: string } }) {
  const updateWithId = updateBlogAction.bind(null, blog.id);
  const [state, formAction, pending] = useActionState(updateWithId, initialState);

  return (
    <form action={formAction} className="bg-white p-[30px] md:p-[40px] rounded-[30px] shadow-[0px_10px_30px_rgba(0,0,0,0.03)] border border-pale-blue">
      {state?.success && (
      ...
      <div className="mb-6">
        ...
        {state?.errors?.name && (
          <p className="text-[#ff0000] text-sm mt-2 ml-4">{state.errors.name[0]}</p>
        )}
      </div>

      <div className="mb-[30px]">
        <label htmlFor="description" className="block text-dark-purple font-bold mb-2">
          Blog Description
        </label>
        <div className="relative">
          <textarea
        ...
        {state?.errors?.description && (
          <p className="text-[#ff0000] text-sm mt-2 ml-4">{state.errors.description[0]}</p>
        )}
      </div>
